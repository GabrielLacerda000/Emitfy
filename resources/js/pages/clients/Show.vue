        ...
        <div class="flex flex-col gap-8 p-6 lg:p-8">
            <div class="mx-auto w-full max-w-7xl">
                    ...
                    <div class="flex items-start gap-5">
                        ...
                        <div class="space-y-1">
                            ...
                            <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-muted-foreground">
                                ...
                                <span class="flex items-center gap-1.5 text-sm font-medium">
                                    <Calendar class="h-4 w-4 opacity-70" /> {{ t('clients.show.joined') }} {{ new Date(client.created_at).toLocaleDateString() }}
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div class="lg:col-span-2 space-y-8">
                        <section>
                            <div class="flex items-center justify-between mb-4">
                                <h2 class="text-sm font-bold uppercase tracking-widest text-muted-foreground">{{ t('clients.show.financialOverview') }}</h2>
                            </div>
                        ...
                        <section class="rounded-3xl border border-border/50 bg-card p-2 shadow-sm">
                            <div class="p-4 flex items-center justify-between">
                                <h2 class="font-bold tracking-tight">{{ t('clients.show.recentInvoices') }}</h2>
                                <Button size="sm" variant="secondary" class="font-bold" @click="redirectToCreateInvoice">{{ t('clients.show.newInvoice') }}</Button>
                            </div>
                    ...
                    <div class="space-y-8">
                        <section>
                            <h2 class="text-sm font-bold uppercase tracking-widest text-muted-foreground mb-4">{{ t('clients.show.clientDetails') }}</h2>
                            <ClientInfoCard
                                :client="client"
        ...
        <Dialog v-model:open="deleteDialogOpen">
            <DialogContent class="sm:max-w-[425px] rounded-xl">
                <DialogHeader>
                    ...
                    <DialogTitle class="text-center text-xl font-bold">{{ t('clients.show.deleteTitle') }}</DialogTitle>
                    <DialogDescription class="text-center">
                        {{ t('clients.show.deleteDesc', { name: client.name, count: invoices.total }) }}
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter class="mt-6 flex flex-col sm:flex-row gap-3">
                    <DialogClose as-child>
                        <Button variant="ghost" class="flex-1 rounded-xl">{{ t('clients.show.goBack') }}</Button>
                    </DialogClose>
                    <Button
                        variant="destructive"
                        ...
                        @click="deleteClient"
                    >
                        {{ deleting ? t('clients.show.deleting') : t('clients.show.confirmDelete') }}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
